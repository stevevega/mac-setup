...
function initializeEmbeddedContentDocuments(getClient) {
    ...
    toDispose.push(vscode_1.workspace.onDidCloseTextDocument(d => {
        if (embeddedContentUri_1.isEmbeddedContentUri(d.uri)) {
            ...
            let vdocUri = embeddedContentUri_1.getEmbeddedContentUri(d.uri.toString(), htmlLanguageId);
    ...
    */
    ...
    ;
    function isPositionPhp(vdocUri, position) {
    ...
    let middleware = {
        provideCompletionItem: (document, position, token, next) => {
            return middleWarePositionalRequest(document, position, () => {
                return next(document, position, token);
            }, isFalseyCompletionResult, (vdoc) => {
                return vscode_1.commands.executeCommand('vscode.executeCompletionItemProvider', vdoc.uri, position);
        ...
        provideDefinition: (document, position, token, next) => {
            return middleWarePositionalRequest(document, position, () => {
                ...
                return vscode_1.commands.executeCommand('vscode.executeDefinitionProvider', vdoc.uri, position).then((def) => {
                    let hostUri = getClient().protocol2CodeConverter.asUri(embeddedContentUri_1.getHostDocumentUri(vdoc.uri));
                    if (!def) {
                        return def;
                    }
        ...
        provideReferences: (document, position, options, token, next) => {
            return middleWarePositionalRequest(document, position, () => {
                ...
                return vscode_1.commands.executeCommand('vscode.executeReferenceProvider', vdoc.uri, position);
            }, [], token);
        },
        /*
        ...
        */
        provideDocumentLinks: (document, token, next) => {
            ...
            return openEmbeddedContentDocument(vdocUri, document.version).then((vdoc) => {
                ...
                return vscode_1.commands.executeCommand('vscode.executeLinkProvider', vdoc.uri);
            });
        },
